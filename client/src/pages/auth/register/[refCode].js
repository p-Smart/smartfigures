...
const Page = ({refCode}) => {
  ...
  const handleSubmit = (values, helpers) => {
    setRegLoading(true)
      ...
      axios.post(`${process.env.NEXT_PUBLIC_API}/register`, formData + `&ref_code=${refCode}`)
      .then( ({data}) => {
...
export const getServerSideProps = async ({query}, res) => {
  const {data} = await axios.get(`${process.env.NEXT_PUBLIC_API}/referrals/verify-referral-code?refCode=${query?.refCode}`)
  if(!data?.refCodeExist){
    return({
      redirect: {
